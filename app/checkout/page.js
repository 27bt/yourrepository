                        ...
                        <ClientOnly>
                            <div className="p-4">
                                <div className="flex items-baseline justify-between text-sm mb-1">
                                    ...
                                    <div>CFA{(cart.cartTotal() / 100).toFixed(2)}</div>
                                </div>
                                ...
                                <div className="flex items-center justify-between my-4">
                                    ...
                                    <div className="text-2xl font-semibold">
                                        CFA{(cart.cartTotal() / 100).toFixed(2)}
                                    </div>
                                </div>
    ...
    const pay = async (event) => {
        ...
        try {
            const paymentResponse = await initiateMoMoPayment(
                ...
                'EUR',
                `${user.id}-${Date.now()}`,
                { partyIdType: 'MSISDN', partyId: user.phone },
                'Payment for order',
                'Order payment'
            );
            ...
            const orderResponse = await fetch('/api/orders/create', {
                method: "POST",
                body: JSON.stringify({
                    stripe_id: paymentResponse.transactionId,
                    name: addressDetails.name,
                    address: addressDetails.address,
                    zipcode: addressDetails.zipcode,
                    city: addressDetails.city,
                            ...
                            <div className="p-4">
                                ...
                                <form onSubmit={pay}>
                                    <button 
                                        ...
                                        disabled={isLoading}
                                    >
                                        {isLoading ? <AiOutlineLoading3Quarters className="animate-spin" /> : 'Confirm and pay'}
                                    </button>
                                </form>
                            </div>
                        </ClientOnly>
